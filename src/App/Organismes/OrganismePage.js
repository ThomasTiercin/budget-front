...
class OrganismePage extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            i: 1,
    ...
    render() {
        const { organismes, remove, message, succed } = this.state;
        let { i } = this.state;
        const content = organismes.map((organisme, index) => 
            (
                <tr key={index}>
                    <th scope="row">{i++}</th>
                    <td><img style={{height: "1.5rem"}} src={organisme.image} /></td>
                    <td>{organisme.nom}</td>
                    <td><Link to={'/editOrganisme/'+organisme.id} style={{color: "#212529"}}><i className="far fa-edit"></i></Link></td>
                    <td><a type="button" onClick={()=>{this.removeLine(organisme.id)}}  style={{color: "#212529"}}><i className="fas fa-trash"></i></a></td>
                </tr>
            )
        )
        return (
            <div className="col-md-12 ">
                <div className="row">
                    <h1 className='col-6'>Organismes</h1>
                ...
                <div><Link to={'/createOrganisme'} style={{color: "#212529"}}><i className="fas fa-plus"></i></Link></div>
