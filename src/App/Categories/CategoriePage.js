...
class CategoriePage extends React.Component {
    ...
    render() {
        const { categories, remove, message, succed } = this.state;
        let { i } = this.state;
        const content = categories.map((categorie, index) => 
            (
            <tr key={index}>
                <th scope="row">{i++}</th>
                <td>{categorie.nom}</td>        
                <td><Link to={'/editCategorie/'+categorie.id} style={{color: "#212529"}}><i className="far fa-edit"></i></Link></td>
                <td><a type="button" onClick={()=>{this.removeLine(categorie.id)}}  style={{color: "#212529"}}><i className="fas fa-trash"></i></a></td>
            </tr>
            )
        )
        return (
            <div className="col-md-12 ">
                <div className="row">
                    <h1 className='col-6'>Catégories</h1>
                ...
                <div><Link to={'/createCategorie'} style={{color: "#212529"}}><i className="fas fa-plus"></i></Link></div>
