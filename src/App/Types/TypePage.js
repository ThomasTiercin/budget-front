...
class TypePage extends React.Component {
    ...
    render() {
        const { types, remove, message, succed } = this.state;
        let { i } = this.state;
        const content = types.map((type, index) => 
            (
            <tr key={index}>
                <th scope="row">{i++}</th>
                <td>{type.nom}</td>                
                <td><Link to={'/editType/'+type.id} style={{color: "#212529"}}><i className="far fa-edit"></i></Link></td>
                <td><a type="button" onClick={()=>{this.removeLine(type.id)}}  style={{color: "#212529"}}><i className="fas fa-trash"></i></a></td>
            </tr>
            )
        )
        return (
            <div className="col-md-12 ">
                ...
                <div><Link to={'/createType'} style={{color: "#212529"}}><i className="fas fa-plus"></i></Link></div>
